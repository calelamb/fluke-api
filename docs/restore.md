...
3. Point a one-off process using the deployed production image at the restored database and run `npm run db:migrate:status`, `npm run db:migrate:deploy`, and `npm run db:verify-seed:runtime`.
